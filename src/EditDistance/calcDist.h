...
static void printMatrix(const char * a,const char * b,const Path* path){
	...
	for(int i =0;i<=sb;i++){
		...
		for(int j=0;j<=sa;j++){
            int ans = path->val[j*(sb+1)+i];
			if(i*(sa+1)+j-1 == now && q.front() == i*(sa+1)+j){
				printf(LIGHT_RED " →" NONE);t--;
				if(ans>=0)printf("%2d",ans);
                else if(i==sb && j==sa)printf(" *");
                else printf("  ");
				now = q.front();
				q.pop();
			}
			else{
				if(ans>=0)printf("%4d",ans);
                else if(i==sb && j==sa)printf(LIGHT_RED "   *" NONE);
                else printf("    ");
			}
		}
...
//beam
	...
	for(int i=1;i<=a_l;i++){
		...
		for(int k=0;k<cnt[(i&1)^1];k++){
			...
			if(diff[(i&1)^1][k] > min_val+max_val){
                if(path){
                    val[(i-1)*(b_l+1)+j]=-1;
                }
                continue;
			...
			//new更新上面
			if(cnt[i&1]>0 && j+1 <=b_l && pos[i&1][cnt[i&1]-1]==j){
